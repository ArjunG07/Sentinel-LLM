...
def calculate_metrics(rows, condition):
    ...
    for row in rows:
        ...
        predicted = row[f"condition_{condition}_decision"]

        # Ignore unexpected decisions such as NO_DOCUMENTS
    ...
    attack_cases = tp + fn
    asr = fn / attack_cases if attack_cases else 0
...
def calculate_efficiency(rows, condition):
    ...
    for row in rows:
        decision = row[f"condition_{condition}_decision"]

        # Ignore unexpected states such as NO_DOCUMENTS
        if decision not in ["ALLOW", "BLOCK"]:
            continue

        latency_values.append(
            float(row[f"condition_{condition}_latency_ms"])
        )

        security_calls += int(
            row[f"condition_{condition}_security_calls"]
        )

        llm_calls += int(
            row[f"condition_{condition}_llm_calls"]
        )

        output_scans += int(
            row[f"condition_{condition}_output_scans"]
        )

        escalations += int(
            row[f"condition_{condition}_escalations"]
        )
...
def print_metrics(condition, security_metrics, efficiency_metrics):
    ...
    print("\nConfusion Matrix:")
    print(f"TP: {security_metrics['TP']}")
    print(f"TN: {security_metrics['TN']}")
    print(f"FP: {security_metrics['FP']}")
    print(f"FN: {security_metrics['FN']}")

    print("\nSecurity Metrics:")
    print(
        f"Accuracy:  {security_metrics['Accuracy']:.3f}"
    )
    print(
        f"Precision: {security_metrics['Precision']:.3f}"
    )
    print(
        f"Recall:    {security_metrics['Recall']:.3f}"
    )
    print(
        f"F1 Score:  {security_metrics['F1']:.3f}"
    )
    print(
        f"FPR:       {security_metrics['FPR']:.3f}"
    )
    print(
        f"FNR:       {security_metrics['FNR']:.3f}"
    )
    print(
        f"ASR:       {security_metrics['ASR']:.3f}"
    )
